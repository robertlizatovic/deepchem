...
class _GraphConvKerasModel2(tf.keras.Model):
  ...
  def __init__(self,
    ...
    All arguments have the same meaning as in GraphConvModel2.
    """
    super(_GraphConvKerasModel2, self).__init__()
    if mode not in ['classification', 'regression']:
      raise ValueError("mode must be either 'classification' or 'regression'")
  ...
  def call(self, inputs, training=False):
    ...
    atom_features = inputs[input_idx]
    degree_slice = tf.cast(inputs[input_idx + 1], dtype=tf.int32)
    membership = tf.cast(inputs[input_idx + 2], dtype=tf.int32)
    ...
    for i in range(len(self.graph_convs)):
      ...
      if self.batch_norms[bn_idx] is not None:
        gc1 = self.batch_norms[bn_idx](gc1, training=training)
      bn_idx += 1
      if self.dropouts[dp_idx] is not None:
        gc1 = self.dropouts[dp_idx](gc1, training=training)
      dp_idx += 1
      gp_in = [gc1, degree_slice, membership] + deg_adjs
      in_layer = self.graph_pools[i](gp_in)
    ...
    dense = self.dense(in_layer)
    if self.batch_norms[bn_idx] is not None:
      dense = self.batch_norms[bn_idx](dense, training=training)
    bn_idx += 1
    if self.dropouts[dp_idx] is not None:
      dense = self.dropouts[dp_idx](dense, training=training)
    dp_idx += 1
    neural_fingerprint = self.graph_gather([dense, degree_slice, membership] +
                                           deg_adjs)
    ...
    for fc_layer in self.fc_layers:
      ...
      if self.batch_norms[bn_idx] is not None:
        fc_in = self.batch_norms[bn_idx](fc_in, training=training)
      bn_idx += 1
      if self.dropouts[dp_idx] is not None:
        fc_in = self.dropouts[dp_idx](fc_in, training=training)
      dp_idx +=1
    fc_out = fc_in
...
class GraphConvModel2(KerasModel):
  """Graph Convolutional Models.
  ...
  def __init__(self,
    ...
    fc_layers: list of int
      Width channels for FC layers after GraphPool
    task_layer_size: int
      Width of each individual task layer 
    dropout: list or float
      the dropout probablity to use for each layer.  The length of this list
      should equal len(graph_conv_layers)+1 (one value for each convolution
      layer, and one for the dense layer).  Alternatively this may be a single
    ...
    use_external_features: bool
      if True, extra molecule level features are expected as input to the model
    uncertainty: bool
      if True, include extra outputs and loss terms to enable the uncertainty
      in outputs to be predicted
    """
    ...
    if mode == "classification":
      ...
      if self.uncertainty:
        ...
        output_types = ['prediction', 'embedding']
        loss = L2Loss()
    super(GraphConvModel2, self).__init__(
        model, loss, output_types=output_types, batch_size=batch_size, **kwargs)
    ...
    for epoch in range(epochs):
      for (X_b, y_b, w_b, ids_b) in dataset.iterbatches(
        ...
        if y_b is not None and self.mode == 'classification':
          y_b = to_one_hot(y_b.flatten(), self.n_classes).reshape(
              -1, self.n_tasks, self.n_classes)
        # assume X_b is an array of objects if using external mol descriptors
        # where the graphconv object is at column idx 0
        if self.model.use_external_features:
          X_b, X_be = X_b[:, 0].flatten(), X_b[:, 1:].astype(float)
        else:
          X_b, X_be = X_b, None
        multiConvMol = ConvMol.agglomerate_mols(X_b)
        n_samples = np.array(X_b.shape[0])
        ...
        for i in range(1, len(multiConvMol.get_deg_adjacency_lists())):
          inputs.append(multiConvMol.get_deg_adjacency_lists()[i])
        # add external descriptors to input if specified
        if X_be is not None:
            inputs = [X_be] + inputs
